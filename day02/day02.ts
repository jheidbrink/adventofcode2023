...
export const is_valid_draw = (draw: Draw, bag_contents: Draw): boolean => {
    return draw.red <= bag_contents.red && draw.green <= bag_contents.green && draw.blue <= bag_contents.blue;
}
...
export const fewest_number_of_cubes_in_bag = (draws: Draw[]): Draw => {
    const maxForEachColor = (drawA: Draw, drawB: Draw): Draw => {
        return { red: Math.max(drawA.red, drawB.red), green: Math.max(drawA.green, drawB.green), blue: Math.max(drawA.blue, drawB.blue)};
    }
